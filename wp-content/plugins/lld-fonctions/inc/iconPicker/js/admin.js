...
miPicker = {
	templates: {},
	wrapClass: '.menu-icons-wrap',
	tinymceClass: '.mce-iconPicker',
	frame: null,
	...
	setIcon: function( $el ) {
		...
		$el.parent().find( 'div._settings input' ).each( function() {
	...
	unsetIcon: function( $el ) {
		...
		$el.parent().find('.iconNameField').attr('value', '');
		$el.parent().find('.iconUnicodeField').attr('value', '' );
		$el.parent().find('.imageUrlField').attr('value', '' );
		$el.parent().find('i._icon').remove();
		$el.parent().find('._select').append(menuIcons.text.select);
		$el.parent().find('._remove').addClass( 'hidden' );
		$el.parent().find('i._icon').removeClass();
		$el.parent().parent().find('img._icon').remove();
	},
	...
	updateField: function( e ) {
		...
		var $el    = $( e.currentTarget ),
		    ...
		    type   = $el.parent().find( 'input._mi-type' ).val(),
		    icon   = $el.parent().find( 'input._mi-icon' ).val(),
		    url    = $el.parent().find( 'input._mi-url' ).val(),
		    iconCode = $el.parent().find('.iconUnicodeField').val(),
		    iconName = $el.parent().find('.iconNameField').val(),
		    imageUrl = $el.parent().find('.imageUrlField').val(),
		    template;

		if ( '' === iconCode || '' === iconName || '' === imageUrl )
		...
		if ( url != '')
		{
			$el.parent().find('.imageUrlField').attr('value', url );
			$el.parent().find('.iconType').attr('value', 'image' );
		}
		else
		{
			$el.parent().find('.iconNameField').attr('value', icon);
			$el.parent().find('.iconUnicodeField').attr('value', fFontAwesomeToUnicode(icon) );
			$el.parent().find('.iconType').attr('value', 'icon' );
