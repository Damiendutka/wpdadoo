...
final class Menu_Icons_Settings {
	...
	public static function init() {
		...
		# Filter to set which icons are applied
		$tabTypeIcons = array();
		$tabTypeIcons = apply_filters('filterIconPickerTypes', $tabTypeIcons);

		foreach ( self::$settings as $key => &$value ) {
			if ( 'global' === $key ) {
				// Remove unregistered icon types.
				$value['icon_types'] = $tabTypeIcons;
			} else {
	...
	public static function _enqueue_assets() {
		...
		$js_data = apply_filters(
			...
			array(
				'text'           => array(
					'title'        => __( 'Select icon', '6tem9Fonction' ),
					'select'       => __( 'Select an icon', '6tem9Fonction' ),
					'remove'       => __( 'Remove', '6tem9Fonction' ),
					'change'       => __( 'Change', '6tem9Fonction' ),
					'all'          => __( 'All', '6tem9Fonction' ),
					'preview'      => __( 'Preview', '6tem9Fonction' ),
					'settingsInfo' => sprintf(
						esc_html__( "Note : The icon size setting will only work into the text editor.", '6tem9Fonction' )
					),
				),
