...
final class Menu_Icons_Picker {
	...
	public static function init() {
		...
		add_action( 'load-post-new.php', array( __CLASS__, '_load_nav_menus' ), 1 );
		add_action( 'load-post.php', array( __CLASS__, '_load_nav_menus' ), 1 );
		// add_filter( 'wp_edit_nav_menu_walker', array( __CLASS__, '_filter_wp_edit_nav_menu_walker' ), 99 );
		add_filter( 'iconPicker', array( __CLASS__, '_fields' ), 10, 4 );
	...
	public static function _fields( $id, $item, $depth, $args ) {
		...
		?>
			<div class="field-icon description-wide menu-icons-wrap" data-id="<?php echo $id['id']; ?>">
				<?php
					/**
...
<?php
					...
					{		
						printf( '<a class="_select button" title="Change">%s</a>', '<i class="_icon fa ' . $id['name'] . '"></i>' );
						printf( '&nbsp;<a class="_remove submitdelete">%s</a>', esc_html__( 'Remove', '6tem9Fonction' ) );
					}
					else if ( isset($id['url']) && $id['url'] != '' )
					{
						printf( '<a class="_select button" title="Change">%s</a>', '<img src="' . $id['url'] . '" height="25" width="25" class="_icon" />' );
						printf( '&nbsp;<a class="_remove submitdelete">%s</a>', esc_html__( 'Remove', '6tem9Fonction' ) );
					}
					else if ( isset($id['unicode']) && $id['unicode'] != '' )
					{		
						printf( '<a class="_select button" title="Change">%s</a>', '<i class="_icon fa">' . '&#x'.$id['unicode'] . '</i>' );
						printf( '&nbsp;<a class="_remove submitdelete">%s</a>', esc_html__( 'Remove', '6tem9Fonction' ) );
					}
					else
					{
						printf( '<a class="_select button">%s</a>', esc_html__( 'Select an icon', '6tem9Fonction' ) );
						printf( '&nbsp;<a class="_remove submitdelete hidden">%s</a>', esc_html__( 'Remove', '6tem9Fonction' ) );
					}
?>				
				</p>
